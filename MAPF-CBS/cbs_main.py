...
def run_agent_sequential_tasks(agent_id=1, max_tasks=5, loop=0):
    ...
    for idx, row in tasks.iterrows():
        ...
        stats.append({
            ...
            "full_path": str(path)  # 좌표 시퀀스 전체 (리스트 형태를 str로)
        })
        curr_pos = goal

    # 전체 path 연결 (중복 없이 이어붙임)
    full_path = [paths[0][0]] if paths else []
    for p in paths:
        full_path.extend(p[1:])

    fig_path = f"results/fig_agent{agent_id}_loop{loop}.png"
    anim_path = f"results/anim_agent{agent_id}_loop{loop}.gif"
    stat_path = f"results/stat_agent{agent_id}_loop{loop}.csv"

    if config.SAVE_FIG:
        plot_map_with_paths(
            my_map, [tuple(map(int, tasks.iloc[0][['start_y','start_x']]))], 
